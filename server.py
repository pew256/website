...
class AdminServer(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_url.path == '/api/projects':
            ...
            if not os.path.exists(config_file):
                default_config = {
                    "hero_headline": {
                        "base_en": "Deterministic Ledger Infrastructure for Mission-Critical Finance.",
                        "fr": "Infrastructure de registre déterministe pour la finance critique.",
                        "es": "Infraestructura de registro determinista para finanzas de misión crítica.",
                        "de": "Deterministische Ledger-Infrastruktur für geschäftskritische Finanzen.",
                        "sync_lock_fr": False, "sync_lock_es": False, "sync_lock_de": False
                    },
                    "hero_subheadline": {
                        "base_en": "We provide protocol-level engineering and hardened middleware for global institutions. We treat MiCA and the GENIUS Act as technical specifications, building high-availability on-ramps that never go offline.",
                        "fr": "Nous fournissons une ingénierie de protocole et un middleware renforcé pour les institutions mondiales.",
                        "es": "Proporcionamos ingeniería a nivel de protocolo y middleware reforzado para instituciones globales.",
                        "de": "Wir bieten Protokoll-Level-Engineering und gehärtete Middleware für globale Institutionen.",
                        "sync_lock_fr": False, "sync_lock_es": False, "sync_lock_de": False
                    },
                    "primary_color": "#506684",
                    "show_pow": True
                }
            ...
            if lang and lang in ['en', 'fr', 'es', 'de', 'ar']:
                flat = {}
                for k, v in config.items():
                    if isinstance(v, dict) and 'base_en' in v:
                        flat[k] = v.get(lang, v.get('base_en', ''))
                    else:
                        flat[k] = v
    ...
    def do_POST(self):
        ...
        if parsed_url.path == '/api/config':
            ...
            import subprocess
            try:
                proc = subprocess.Popen(['python3', 'translate.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = proc.communicate(input=json.dumps({"config": payload}).encode('utf-8'))
                translated_config = json.loads(stdout.decode('utf-8'))
            except Exception as e:
                print(f"Translation sync failed: {e}")
                translated_config = payload

            with open(config_file, 'w') as f:
                json.dump(translated_config, f, indent=4)
                
            # Perform git operations inline for save transparency
            try:
                subprocess.run("git add config.json", shell=True, check=True)
                subprocess.run("git diff --staged --quiet || git commit -m 'Auto-sync translation inline update'", shell=True, check=True)
                subprocess.run("git push -u origin main", shell=True, check=True)
            except Exception as e:
                print(f"Failed to push translations to git: {e}")
                
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success", "config": translated_config}).encode())

        elif parsed_url.path == '/api/projects/create':
            ...
            if published_takes and published_takes != 'none':
                ...
                # TRANSLATION PIPELINE INJECTION (PHASE 9)
                # Before generating assets, synchronously lock and translate the new take content
                config_file = os.path.join(os.getcwd(), 'config.json')
                try:
                    with open(config_file, 'r') as f:
                        current_config = json.load(f)
                        
                    # Inject translatable nodes into dictionary
                    current_config[f"take_{timestamp}_subject"] = {"base_en": subject, "sync_lock_fr": False, "sync_lock_es": False, "sync_lock_de": False, "sync_lock_ar": False}
                    current_config[f"take_{timestamp}_bull"] = {"base_en": bull_case, "sync_lock_fr": False, "sync_lock_es": False, "sync_lock_de": False, "sync_lock_ar": False}
                    current_config[f"take_{timestamp}_bear"] = {"base_en": bear_case, "sync_lock_fr": False, "sync_lock_es": False, "sync_lock_de": False, "sync_lock_ar": False}
                    
                    import subprocess
                    proc = subprocess.Popen(['python3', 'translate.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    stdout, stderr = proc.communicate(input=json.dumps({"config": current_config}).encode('utf-8'))
                    translated_config = json.loads(stdout.decode('utf-8'))
                    
                    with open(config_file, 'w') as f:
                        json.dump(translated_config, f, indent=4)
                except Exception as e:
                    print(f"Failed to auto-translate Take: {e}")
    ...
    <p>If you are not redirected automatically, <a href="https://pew256.com/index.html#{target_img_prefix}">click here to read the insight</a>.</p>
            ...
            if published_takes and published_takes != 'none':
                try:
                    ...
                    cmd = f"{sys.executable} scripts/generate_diffusion_assets.py --id {timestamp} --project '{project_name}' --mode {gen_mode} && git add assets/published_journal.json insights/ assets/shares/ assets/journal/ config.json && (git diff --staged --quiet || git commit -m 'Auto-publish translated insight HTML and Images atomically') && git push -u origin main"
                    subprocess.Popen(cmd, shell=True)
                except Exception as e:
                ...
                try:
                    subprocess.run("git add assets/published_journal.json insights/ assets/shares/ config.json", shell=True, check=True)
                    subprocess.run("git diff --staged --quiet || git commit -m 'Unpublish insight HTML and delete legacy screenshots'", shell=True, check=True)
                    subprocess.run("git push -u origin main", shell=True, check=True)
                except Exception as e:
